...
**If you want to learn more about MaxLeap Code Service SDK, please check[Java Guide － Cloud Code](ML_DOCS_GUIDE_LINK_PLACEHOLDER_JAVA) for more details.**
...
##WhiteList
Once a job is added into the whitelist, MaxLeap will not perform verification for `X-®-AppId` and `X-ML-APIKey` in `Http Request Head`, while calling that job.
