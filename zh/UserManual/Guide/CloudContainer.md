...
### 二级子域名
在【应用设置 -> 系统设置】下，配置云容器二级域名；当您部署程序后，可以通过二级域名可以直接访问，也可以在您的顶级域名中配置 CName，指向 MaxLeap 平台的二级域名，然后通过您的顶级域名访问，如下图所示

可以直接访问

http://weixg.maxdocker.cn


![imgCCVersionList](../../../images/web_container1.png)

### 服务器程序版本
在【开发中心 -> 云容器 -> 版本】下，我们可以查看上传至该应用下的所有的服务器程序。该模块包含以下列：
...
####上传服务器程序
在【开发中心 -> 云容器 -> 版本】下，我们可以上传该应用下的服务器程序。根据部署程序类型，选择点击 **上传静态网站** / **上传JAVA项目** / **上传PHP项目** 来上传并配置，详情如下：
...
### 日志
在【开发中心 -> 云容器 -> 日志】下，您可以查看程序运行日志，查看以下信息：
...
在【开发中心 -> 云容器 -> 实例】下，您可以查看每个实例的运行状况，包括：
...
## 重要提示
云容器运行时，保存在本地文件系统的所有文件，<font color=red>在重新部署或者重新发布新版本后，将会全部消失</font>。建议所有的数据存储基于 MaxLeap 云数据库或者 MaxLeap 云数据源；所有的文件存储基于 MaxLeap 云数据库的File存储类型或者其它第三方云存储。
