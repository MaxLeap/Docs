...
最后，你能说微信是什么东西么，我是说不清楚，太多了。但是，至少每个用微信的人，都知道微信能加好友，并且和好友聊天。

## 我们要做哪些

今天我们不用去讨论再做一个微信是否可行，马云已经试过了，这事还真有点难，今天我们只从**技术和成本角度**去讨论快速简单实现一个像微信这样的社交应用。
...
好，核心业务需求确定了，终于知道我们做什么了，要开始动手了么，别急，让我们再来稍微详细理一下要做哪些具体业务功能：
...
作为一个移动应用还需要确定支持平台及用户规模，我们就以支持 iOS 和 Android 两个平台，日活跃 10 万的标准为目标。

### 技术方面也要考虑

我们已经大概知道要做什么以及目标用户规模，就够了么？一个互联网应用，技术需求也是需要重点考虑的，不然你懂的。
...
重点终于到了，我们已经清楚要做什么了，在开始做之前，先简单列一下需要做的有哪些：
...
假如我们想所有东西都自己做，自己组建技术团队，从前端到后端，搭建完整服务，我们不缺资金，也有时间，Ok，我们需要哪些？
...
其实，像实时聊天模块，我们可以直接使用第三方实时通讯后端云服务，不用再自己完成从 0 开始搭建，让专业的人做专业的事情，各种省心、省事、省力，我们自己实现账号系统和朋友圈就可以，后端资源和研发成本可以减少一些。

先来稍微了解一下后端云的概念，BaaS(Backend as a Service)，后端即服务，为移动应用和游戏开发者提供后端服务，服务包括结构化的数据存储、用户和权限管理、文件存储、云参数、云代码、推送、支付、实时通信等。BaaS 产品目的是降低开发者成本，简化应用开发流程，让开发者只需专注于具体业务和逻辑的实现，无需关心后端基础设施构建、运维、服务器托管、网络、性能调优等工作。
...
MaxLeap  | 支持数据存储，即时通讯等移动应用一站式后端云服务平台  | [https://maxleap.cn/](https://maxleap.cn/)
...
既然实时通讯可以使用第三方后端云服务搭建，账号系统和朋友圈模块呢，有没有第三方的后端云服务平台可以直接使用？

目前市场上主要有三家公司提供移动应用后端数据存储等后端云服务：

服务商  | 简介 | 官网
------------- | ------------- | ---------
MaxLeap  | 支持数据存储，数据分析，即时通讯等移动应用一站式后端服务平台  | [https://maxleap.cn/](https://maxleap.cn/)
Bmob  | 移动应用后端云提供商  | [http://bmob.cn/](http://bmob.cn/)
LeanCloud  | 移动应用的一站式云端服务  | [https://leancloud.cn](https://leancloud.cn/)
...
想快速验证想法，或者没有时间和资金搭建完整团队，后端云也许比较适合。
